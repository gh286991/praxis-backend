...
export class ExecutionController {
  ...
  executeStream(
    ...
    return from(
      this.executionService.executePythonStream(body.code, body.input),
    ).pipe(
      switchMap((obs) => obs),
  ...
  async submitStream(
    ...
    try {
      ...
      if (!question) {
        res.write(
          `data: ${JSON.stringify({ status: 'error', message: 'Question not found' })}\n\n`,
        );
        res.end();
        return;
      }
  ...
  async submitCode(
    ...
    const [testResult, semanticResult] = await Promise.all([
      ...
      this.executionService.evaluateSubmission(
        body.code,
        question.testCases || [],
      ),
      ...
      this.geminiService.checkSemantics(
        {
          ...
          difficulty:
            (question.difficulty as 'easy' | 'medium' | 'hard') || 'easy',
